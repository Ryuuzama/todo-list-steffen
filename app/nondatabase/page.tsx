...
function AddTask({
  ...
  //React.Dispatch is used in case we ever pass in just an array or a function that returns a new array.

  const [userInputTaskName, setUserInputTaskName] = useState<string>("");
  const [userInputTaskDescription, setUserInputTaskDescription] =
    useState<string>("");
  const makeNewTask = () => {
          ...
          {filteredTasks.map((task) => (
            ...
            >
              ...
              <td>
                <button
                  className="btn"
                  onClick={() => handleDelete({ tasks, setTasks, id: task.id })}
                >
                  Delete
                </button>
              </td>
