    ...
    <div> {/*create task*/}
          ...
          <thead>
            <tr className="p-20">
              ...
              <td>
              <button className="text-white text-base btn btn-wide bg-primary hover:bg-accent"onClick={() => handleAddTask(userInputTaskName,userInputTaskDescription )}>Add Task</button>
              </td>
            </tr>
          </thead>
        </table>
          ...
          <tr 
            ...
            <td>
              {editingNameId === task.id ? (
                <input 
                  ...
                  onBlur={() => handleNameSubmit(task.id, { title: editedUserInputTaskName })}
                  onKeyDown={(event) => {
                    if (event.key === "Enter") {
                      handleNameSubmit(task.id, { title: editedUserInputTaskName });
                    }
                  }}
                  autoFocus
                                 />
            ...
            {editingDescriptionId === task.id ? (
                <input 
                  ...
                  onBlur={() => handleDescriptionSubmit(task.id, { description: editedUserInputTaskDescription })}
                  onKeyDown={(event) => {
                    if (event.key === "Enter") {
                      handleDescriptionSubmit(task.id, { description: editedUserInputTaskDescription });
                    }
                  }}
                  autoFocus
                                 />
            ...
            <td>
            <input type="checkbox" checked={task.completed} className="checkbox bg-base-100" onChange={() => handleCompleted(task.id, { completed: !task.completed })} />
            </td>
